...
export const editCourse=catchAsyncError(async(req,res,next)=>{
    try {
        ...
        if(thumbnail && !thumbnail.startsWith('https')){
            ...
            const myclound= await cloudinary.v2.uploader.upload(thumbnail ,{
                folder:"courses",
...
export const getSingleCourse=catchAsyncError(async(req,res,next)=>{
  ...
  if(isCacheExist){
    ...
    res.status(200).json({
     ...
     const course=await courseModel.findById({_id:courseId}).select(" -courseData.videoUrl -courseData.suggestion -courseData.links -courseData.questions")
